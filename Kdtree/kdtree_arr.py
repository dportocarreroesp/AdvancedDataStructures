...
def closest_point(node, point, depth = 0, best = None):
    ...
    if (point[axis] < node.point[axis]):
        ...
        if (abs(LD(point[axis],node.point[axis])) < distanceSquared(point, best)):
...
def knn(node, query_point,nodes_visited, depth = 0):
    ...
    if ((len(bpq.queue) < bpq.size) or (abs(LD(query_point[axis],node.point[axis])) < bpq.queue[len(bpq.queue) - 1][1])):
